...
router.get('/app/login', function (req, res, next) {
    let cfg = req.nconf.connections.get('dbs');
    let dbs = [];
    for (let key in cfg) {
...
router.post('/app/login_action', function (req, res, next) {
    ...
    if (cfg[req.body.inputDB]) {
        ...
        try {
            connPool.addConnection({connName: req.body.inputDB, connString: dbstr,connOptions:connOptions}, req.app, function (err, data) {
                if (err) {
                    res.render('login', {
                        message: err.message,
                        dbList: dbs,
